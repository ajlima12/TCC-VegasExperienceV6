...
const TransporteCrud = () => {
  ...
  const [tarifa_tpt, setTarifa] = useState('');
  ...
  const handleCriarTransporte = async () => {
    try {
      await firestore().collection('transporte').add({ nome_tpt, descricao_tpt, cod_tpt, tarifa_tpt });
      setNome('');
      setDescricao('');
      setCodTpt('');
      setTarifa('');
  ...
  const handleEditarTransporte = async () => {
    ...
    try {
      await firestore().collection('transporte').doc(editando.id).update({ nome_tpt, descricao_tpt, cod_tpt, tarifa_tpt });
      setNome('');
      setDescricao('');
      setCodTpt('');
      setTarifa('');
  ...
  const handleEditar = (transporte) => {
    if (transporte) {
      ...
      setTarifa(transporte.tarifa_tpt);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.table}>
        <View style={styles.tableRow}>
          ...
          <View style={[styles.tableCell, styles.tableCellHeader, styles.centeredHeader]}>
            <Text style={styles.tableCellHeaderText}>Tarifa</Text>
          </View>
        </View>
        {transportes.map((item) => (
          <TouchableOpacity key={item.id} onPress={() => handleEditar(item)}>
            <View style={styles.tableRow}>
              ...
              <View style={[styles.tableCell, styles.centeredCell]}>
                <Text style={styles.tableCellText}>{item.tarifa_tpt}</Text>
              </View>
            </View>
          </TouchableOpacity>
        ))}
